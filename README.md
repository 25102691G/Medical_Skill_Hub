...
`compile_skill.py` 是独立的 guideline skill 编译入口，不接入 `main.py` 诊断主流程。它将 PDF 指南编译成 `skills/<skill-name>/` 目录，输出结构与现有 guideline skills 保持一致：

```text
skills/<skill-name>/SKILL.md
skills/<skill-name>/agents/openai.yaml
skills/<skill-name>/references/guideline-full-text.md
skills/<skill-name>/references/recommendations-index.md
skills/<skill-name>/scripts/search_guideline.py
```

默认流程会调用 MinerU 解析 PDF，生成 `guideline-full-text.md`，再调用 Skill Compiler Agent 从全文中自动生成 `recommendations-index.md` 重要信息索引和 skill 元数据。索引内容由 LLM 根据全文决定，不限于固定推荐意见表格，可包含诊断标准、鉴别诊断、检查、治疗、监测和随访等关键内容：

```bash
python compile_skill.py --pdf path/to/guideline.pdf --skill-name new-guideline-skill
```

如果已经有 MinerU 解析后的 Markdown，可跳过 PDF 解析：

```bash
python compile_skill.py --full-text-md path/to/guideline-full-text.md --skill-name new-guideline-skill
```
...
```bash
export MINERU_COMMAND="mineru -p {input} -o {output} -b pipeline"
python compile_skill.py --pdf path/to/guideline.pdf --skill-name new-guideline-skill
```
...
DeepSeek 分支会使用普通 Chat Completions 文本 JSON 输出，并在本地用 Pydantic 解析，不使用 OpenAI structured `response_format`。
...
目标目录已存在时，脚本会停止；确认需要覆盖时再加 `--force`。
