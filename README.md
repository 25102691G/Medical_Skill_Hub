...
前端开发服务已在 `vite.config.js` 中使用文件轮询模式，避免共享服务器的 Linux `inotify` watcher 配额耗尽时出现 `ENOSPC: System limit for number of file watchers reached`。轮询间隔为 500 毫秒，因此不需要修改系统级 `sysctl` 配置。

然后访问 `http://localhost:5173`。聊天使用方式：

1. 发送患者病史、体征或检查结果；每条普通消息都会追加到当前线程的病例信息。
2. 可以分多条消息继续补充资料。
3. 发送 `开始诊断`、`重新诊断`、`诊断` 或 `/diagnose`，运行完整诊断流水线。
4. 发送 `清空病例`、`重置病例`、`/clear` 或 `/reset`，清除当前线程累计的病例信息。

ChatKit 控件固定使用简体中文。开始诊断后，聊天界面会用中文实时显示当前诊断
阶段和诊断轮次，并在全部阶段结束后显示中文诊断结果。用于 PubMed 等数据源的
内部检索词和中间结果仍可使用英文，但不会作为调试 JSON 展示在聊天界面中；
命令行的 `--debug` 行为不受影响。当前主流程中已注释的 Similar Case Retrieval
Agent 不会运行，也不会显示在进度信息中。

当前版本使用内存存储保存 ChatKit 线程、消息和病例信息，服务重启后数据会清空；附件上传尚未启用。用于生产环境时，应将 `InMemoryChatKitStore` 替换为数据库存储，并在 `/chatkit` 接口增加用户身份认证和访问控制。

如果前端显示 `OpenAI API 额度不足`，并且后端日志中的错误代码为 `insufficient_quota`，需要检查 `.env` 中 API Key 所属 OpenAI API 项目的余额、Billing 和使用预算。ChatGPT 订阅不包含 API 调用额度；更新 API Key 或充值后，需要重启 `run_chatkit.sh` 使配置重新加载。该错误不是普通的每分钟请求限制，重复请求不会自动恢复。
