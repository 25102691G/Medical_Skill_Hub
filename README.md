...
3. 相似病例归纳：将 `search_queries` 传入 Similar Case Retrieval Agent，生成每条查询对应的相似病例诊断归纳。
4. 最终诊断：Digestive Diagnosis Agent 结合病例文本、检索规划结果、知识检索结果、相似病例结果和可用 sandbox skills 输出 topk 疑似诊断。
...
Digestive Diagnosis Agent 会在最终诊断阶段检查 `skills/` 中是否存在与病例、检索规划、知识检索或相似病例结果相关的疾病指南 skill。存在对应 skill 时，通过 OpenAI Agents SDK sandbox capability 挂载到 sandbox 内的 `.agents` 自动发现目录，并按需调用 `load_skill` 加载。
