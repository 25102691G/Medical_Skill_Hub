...
BM25 仅对英文和数字分词，并排除原始分数小于等于 0 的病例；统一语料分别执行
BM25 和 Dense Retriever 检索，再仅使用两路各自的 Top-N 候选执行 RRF 融合排名。
某病例未进入其中一路的 Top-N 时，该分支不为其计算 RRF 分数。最终按相关性顺序
输出最多 10 条病例对应的 `hadm_id`、
...
启用 `--debug` 时，终端的标准错误流会输出统一检索语料的 BM25、Dense 和 RRF Top-K
排名。BM25/Dense 明细包括查询文本、住院号、出院疾病和原始分数；RRF 明细
额外包括最终 RRF 分数及两路候选排名，未进入某路 Top-N 的排名为 `null`。未执行
的检索分支会输出跳过原因。
ChatKit 前端也会在每轮相似病例检索完成后展示同一组三路排名及跳过原因，该展示通过
...
`search_planning_result.similar_case_queries`、BM25/Dense/RRF 排名明细
...
`discharge_texts`。运行时终端仍会输出 BM25、Dense 和 RRF 排名明细及跳过原因。
...
BM25 首次运行时对完整病例库分词并将索引缓存到
`database/mimic_iv_similar_case_bm25.pkl`；病例库未变化时，后续运行直接加载该索引。
Dense Retriever 默认使用 `BAAI/bge-m3`，首次运行时由 Transformers 加载模型，并将
病例库向量缓存到 `database/mimic_iv_similar_case_embeddings.pt`。模型和向量相似度计算默认在 GPU 上运行，
