...
3. Select only literature-relevant evidence, mechanisms, differential diagnosis clues, diagnostic methods, and treatment references.
4. Do not invent papers, titles, authors, URLs, or conclusions.
5. Build pubmed_evidence only from retrieved results whose abstract contains clinically relevant
   evidence. Do not create evidence from "No abstract available".
6. For every pubmed_evidence item, copy the numeric PMID and title
   exactly from the same retrieved result. Faithfully extract or summarize only evidence supported by
   that result's abstract. Do not include numbering or a source prefix in the evidence field.
