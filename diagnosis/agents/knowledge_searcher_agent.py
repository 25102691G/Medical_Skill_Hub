...
## KNOWLEDGE SEARCHER INSTRUCTIONS

You are a clinical Knowledge search specialist in gastroenterology.

### 1. Objective

Read the provided search_queries generated by the Gastroenterology Search Planning Agent and review the
provided PubMed search results.

Select the individual abstract sections whose publication title and section text are relevant to one
or more provided search queries.

### 2. Selection Requirements

A PMID retrieved by multiple search queries should receive additional consideration because repeated
...
Evaluate each abstract section separately. Selecting one section from a publication does not mean that
the publication's other sections should also be selected.

Return each selected PMID and section_index pair only once, in the order of its first appearance in
the provided results.

Copy each selected numeric PMID and section_index exactly from the provided results. Do not invent or
modify either value.

### 3. Output Requirements

Output only selected_sections.

Do not output, copy, rewrite, summarize, or quote any query, title, section text, URL, evidence,
explanation, or unselected PubMed section.
""".strip()
...
class _RateLimitedPubMedRetriever(PubMedRetriever):
    def _parse_article(self, uid: str, text_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        raw_sections = (
            [abstract_text]
            if isinstance(abstract_text, (str, dict))
            else abstract_text or []
        )
        ...
        for section in raw_sections:
            ...
            text = str(text).strip()
            if text:
                article["AbstractSections"].append(
                    {
                        "section_index": len(article["AbstractSections"]) + 1,
                        "text": text,
                    }
                )
        article["Summary"] = ""
...
def _document_to_result(document: Any) -> dict[str, Any]:
    ...
    return {
        "pmid": str(pmid),
        "title": str(title),
        "abstract_sections": metadata.get("AbstractSections", []),
        "url": str(url),
    }
