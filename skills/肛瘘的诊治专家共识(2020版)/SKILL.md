...
description: "类别：肠的其他疾病。用于肛瘘（Anal fistula）的诊断、鉴别诊断、治疗和随访，仅在病例候选诊断包含该疾病时使用。"
---
...
1. 先读取 `references/recommendations-index.md`，定位相关推荐、诊断标准、鉴别诊断、检查、治疗、监测、随访等重要信息。
2. 再读取 `references/guideline-full-text.md` 中的相关内容，补充适用人群、限制条件、解释依据和上下文。
3. 如果问题没有明显对应推荐，使用 `scripts/search_guideline.py` 进行关键词搜索。
...
- `references/recommendations-index.md`：LLM 根据全文自动生成的重要信息索引，用于定位推荐、诊断标准、鉴别诊断、检查、治疗、监测和随访等关键内容。
