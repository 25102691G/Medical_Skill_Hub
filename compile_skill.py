...
DEFAULT_MINERU_COMMAND = "mineru -p {input} -o {output} -b pipeline"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile a PDF guideline into a local skill directory.")
    parser.add_argument("--pdf", type=Path, help="Source PDF file. Required unless --full-text-md is provided.")
    parser.add_argument(
        "--full-text-md",
        type=Path,
        help="Existing MinerU Markdown output. When provided, PDF parsing is skipped.",
    )
    parser.add_argument("--skill-name", help="Target skill directory name. Defaults to the LLM suggestion.")
...
def _validate_inputs(args: argparse.Namespace) -> None:
    if not args.pdf and not args.full_text_md:
        raise SystemExit("Error: --pdf or --full-text-md is required.")
...
def _slugify_skill_name(value: str) -> str:
    ...
    if not slug:
        raise SystemExit("Error: skill name is empty after normalization.")
    return slug
...
def _run_mineru(pdf_path: Path, command_template: str) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="skill-compiler-mineru-") as temp_dir:
        ...
        markdown_path = _find_mineru_markdown(output_dir, pdf_path.stem)
...
def _write_skill_directory(
    ...
    *,
    ...
    if skill_dir.exists() and not force:
        raise SystemExit(f"Error: target skill directory already exists: {skill_dir}. Use --force to overwrite.")
    ...
    _write_text(skill_dir / "SKILL.md", _render_skill_md(skill_name, result))
...
def main() -> int:
    ...
    if args.full_text_md:
        full_text = args.full_text_md.read_text(encoding="utf-8")
    else:
        full_text = _run_mineru(args.pdf, args.mineru_command)

    result = compile_guideline_text(full_text)
    skill_name = _slugify_skill_name(args.skill_name or result.skill_name_suggestion)
    skill_dir = args.skills_dir / skill_name
    _write_skill_directory(skill_dir, skill_name, full_text, result, force=args.force)
